...
SIM8_ftk <- function(N_sim,n_pts,N_sites,follow_up,N_cov,Test_set_nr_pts,predtimes,True_beta,lambda=0.5,gammas=2,ayear=365.25,mean_val=0,SD_val=1,
                     envv=ls(globalenv())){
  ...
  PS <- foreach(
    ...
    Test_set_list <- lapply(X=1:N_sites,FUN =  function(X){sim_fkt_sim8(n_pat = 1000/N_sites, #To ensure an equal number of pts from each site, and that the total number of pts are 1000
                                                                   max_time = max(predtimes),
                                                                   uni_max_time = follow_up, #followup
                                                                   True_beta =True_beta[[X]],
                                                                   SD_Val = SD_val,
...
mean_val <- 0
SD_val <-1
True_beta =list(Beta_1=c(X1=log(1.1),X2=log(0.9),X3=log(1.9),X4=log(0.8)),
                Beta_2=c(X1=log(1.2),X2=log(0.8),X3=log(2.0),X4=log(0.7)),
                Beta_3=c(X1=log(1.3),X2=log(1.0),X3=log(2.1),X4=log(0.6)),
                Beta_4=c(X1=log(1.4),X2=log(0.4),X3=log(1.5),X4=log(1.0)))
...
saveRDS(sim8_test,"./Simulation 8 - Beta/sim8_test.rds")
############
### Test ###
############
C_index_sim4 <- C_index_ceaning_fkt(df = sim8_test)
IBS_sim4 <- IBS_df_cleaning_fkt(df = sim8_test)
ABS_Surv_diff_sim4 <- ABS_AVG_SD_fkt(df = sim8_test)

C_index_S4 <- Cindex_box_plot_fkt(C_index_res =C_index_sim4,title="Simulation 4",exclude_y_axis = T,ylim = c(0.50,0.9) )
IBS_S4 <- IBS_box_plot_fkt(IBS_sim4,title="Simulation 4",exclude_y_axis = T ,ylim = c(0.10,0.15) )
